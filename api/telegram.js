  ...
  return {
    inline_keyboard: [
      ...
      [
        ...
        { text: "✍️ Mensaje libre", callback_data: `mensaje_libre:${userId}` }
      ]
    ]
  };
}
...
También podés usar el botón:
✍️ Mensaje libre`);
    return;
  }

  if (command === "/cancelar") {
    adminSessions[ADMIN_ID] = null;
    await sendTelegram(ADMIN_ID, "✅ Acción cancelada.");
    return;
  }
...
async function handleCallback(callbackQuery) {
  ...
  if (action === "rechazar") {
    ...
    await sendWhatsApp(userId, `❌ Solicitud rechazada

Un administrador revisó tu solicitud y no pudo ser aprobada.

Si creés que se trata de un error, podés volver a comunicarte con administración.`);
...
Canal: ${r[20] || "NO"}`);
  ...
  if (action === "mensaje_libre") {
    ...
    await sendTelegram(
      ADMIN_ID,
      `✍️ Escribí el mensaje que querés enviar al usuario <code>${userId}</code>.
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (adminSessions[ADMIN_ID]?.step === "mensaje_libre" && text && text !== "/cancelar") {
      const userId = adminSessions[ADMIN_ID].targetUserId;

      await sendWhatsApp(userId, text);
      await sendTelegram(ADMIN_ID, `✅ Mensaje libre enviado a <code>${userId}</code>.`);
